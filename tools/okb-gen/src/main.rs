...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let password = SecureStr::new(PASSWORD.to_string());
    let output_dir = Path::new(OUTPUT_DIR);

    // Generate 3 normal samples
    println!("Generating normal .okb samples...");

    let basic_path = output_dir.join("okb_basic.okb");
    let basic_size = encrypt_and_write_okb(&basic_payload(), &password, &basic_path)?;
    println!(
        "  Generated: {} ({} bytes)",
        basic_path.display(),
        basic_size
    ...
    let mixed_size = encrypt_and_write_okb(&mixed_types_payload(), &password, &mixed_path)?;
    println!(
        "  Generated: {} ({} bytes)",
        mixed_path.display(),
        mixed_size
    ...
    let edge_size = encrypt_and_write_okb(&edge_cases_payload(), &password, &edge_path)?;
