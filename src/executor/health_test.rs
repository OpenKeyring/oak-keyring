...
fn persist_writes_states_to_db_and_returns_deltas_for_new_evaluations() {
    ...
    let states = executor.vault_mut().unwrap().list_record_health_states().expect("list");
    assert_eq!(states.len(), 2);
...
fn schedule_resync_marks_pending_sync_without_changing_record() {
    ...
    let rec_before = executor.vault_mut().unwrap().get_stored_record(id).expect("get");
    ...
    let rec_after = executor.vault_mut().unwrap().get_stored_record(id).expect("get");
...
fn no_version_bump_when_no_health_deltas() {
    ...
    let version_before = executor.vault_mut().unwrap().get_stored_record(id).expect("get").version;

    // Empty deltas — no changes.
    schedule_health_resync_for_records(&mut executor, &[]).expect("schedule");

    let version_after = executor.vault_mut().unwrap().get_stored_record(id).expect("get").version;
...
fn health_check_completed_persists_record_health_states_to_db() {
    ...
    let states_before = executor.vault_mut().unwrap().list_record_health_states().expect("list");
    ...
    let states_after = executor.vault_mut().unwrap().list_record_health_states().expect("list");
    assert_eq!(
        states_after.len(),
        2,
...
fn health_report_restores_after_simulated_restart() {
    ...
    executor.vault_mut().unwrap().set_last_health_check_at(ts).expect("set");
...
fn delete_record_cleans_up_health_state() {
    ...
    executor.vault_mut().unwrap().soft_delete_record(id).expect("soft delete");
...
fn hibp_skip_after_full_scan_and_persist() {
    ...
    let version = executor.vault_mut().unwrap().get_stored_record(id).expect("get").version;
...
fn health_writeback_does_not_change_record_version() {
    ...
    let v1_before = executor.vault_mut().unwrap().get_stored_record(id1).expect("get").version;
    let v2_before = executor.vault_mut().unwrap().get_stored_record(id2).expect("get").version;
    ...
    let v1_after = executor.vault_mut().unwrap().get_stored_record(id1).expect("get").version;
    let v2_after = executor.vault_mut().unwrap().get_stored_record(id2).expect("get").version;
